...
public class EnvironmentListener {
	...
	@KafkaListener(topics = "${kafka.topic-environment}", containerFactory = "environmentKafkaListenerContainerFactory")

	public void userListner(@Payload Environment schedule, @Header(KafkaHeaders.OFFSET) Integer offset,
	                @Header(KafkaHeaders.RECEIVED_PARTITION_ID) int partition,
	                @Header(KafkaHeaders.RECEIVED_TOPIC) String topic) {
		log.info("Processing topic = {}, partition = {}, offset = {}, workUnit = {}", topic, partition, offset,
		                schedule);
		record.setEnv(schedule);
